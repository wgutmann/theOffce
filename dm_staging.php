<html>
	<head>
		...
		if( isset($_GET['michael_era'])){
			$michael_era = $_GET['michael_era'];
			$filterQuery = $filterQuery . buildQuery($michael_era, "3");
		}
		if( isset($_GET['post_michael'])){
			$post_michael = $_GET['post_michael'];
			$filterQuery = $filterQuery . buildQuery($post_michael, "3");
		}
		if( isset($_GET['pre_sabre'])){
			$pre_sabre = $_GET['pre_sabre'];
			$filterQuery = $filterQuery . buildQuery($pre_sabre, "3");
		}
		if( isset($_GET['sabre_era'])){
			$sabre_era = $_GET['sabre_era'];
			$filterQuery = $filterQuery . buildQuery($sabre_era, "3");
		}
		if( isset($_GET['post_sabre'])){
			$post_sabre = $_GET['post_sabre'];
			$filterQuery = $filterQuery . buildQuery($post_sabre, "3");
		}
		if( isset($_GET['pre_mspc'])){
			$pre_mspc = $_GET['pre_mspc'];
			$filterQuery = $filterQuery . buildQuery($pre_mspc, "3");
		}
		if( isset($_GET['mspc_era'])){
			$mspc_era = $_GET['mspc_era'];
			$filterQuery = $filterQuery . buildQuery($mspc_era, "3");
		}
		if( isset($_GET['post_mspc'])){
			$post_mspc = $_GET['post_mspc'];
			$filterQuery = $filterQuery . buildQuery($post_mspc, "3");
		}
...
//this is bullshit
			if($filter ==3){
				if($GLOBALS["FILTERFLAG"] == FALSE){
					$filterQuery = " CATEGORY LIKE '%" . $sql . "%'";
					$GLOBALS ["FILTERFLAG"] = TRUE;
					return $filterQuery;
				}else{
					$filterQuery = $filterQuery . " OR CATEGORY LIKE '%" . $sql ."%'";
					return $filterQuery;
				}
			}
		}
	...
	<body>
		...
		<form action="dm_staging.php" method="get" name="filters">
			...
			<table border ="0" cellpadding="0" cellspacing="5" style="font-size: 1.4em">
				...
				<tr>
					<td>Michael Scott Era</td>
					<td><input type="checkbox" name='michael_era' value='michael_era'></td>
				</tr>
				<tr>
					<td>Post Michael Scott</td>
					<td><input type="checkbox" name="post_michael" value="post_michael"></td>
				</tr>
				<tr>
					<td>Pre-MSPC</td>
					<td><input type="checkbox" name="pre_mspc" value="pre_mspc"></td>
				</tr>
				<tr>
					<td>Michael Scott Paper Company Era</td>
					<td><input type='checkbox' name="mspc_era" value="mspc_era"></td>
				</tr>
				<tr>
					<td>Post-MSPC</td>
					<td><input type="checkbox" name="post_mspc" value="post_mspc"></td>
				</tr>
				<tr>
					<td>Pre-Sabre</td>
					<td><input type="checkbox" name="pre_sabre" value="pre_sabre"></td>
				</tr>
				<tr>
					<td>Sabre Era</td>
					<td><input type="checkbox" name="sabre_era" value="sabre_era"></td>
				</tr>
				<tr>
					<td>Post Sabre</td>
					<td><input type="checkbox" name="post_sabre" value="post_sabre"></td>
				</tr>
				<tr>
